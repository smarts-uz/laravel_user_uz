                ...
                <div x-show="showModal"
                    ...
                    <div x-show="showModal" class="bg-white rounded-xl shadow-2xl p-6 sm:w-10/12 lg:w-5/12 mx-10"
                        ...
                        <div class="mx-auto pl-5 py-5 text-black">
                            ...
                            <a class='text-sm sm:text-xl text-green-500 hover:text-red-600 send-email border-b sent-email border-dotted @if($errors->has('email') || session()->has('email-message')) hidden @endif border-gray-700 cursor-pointer'
                               href="{{route('login.send_email_verification')}}">{{__('Отправить новое письмо для подтверждения почты')}}</a><br>


                            <a class='text-sm sm:text-xl text-green-500 hover:text-red-600 border-b border-dotted border-gray-700 @if($errors->has('email') || session()->has('email-message') ) hidden @endif change-email cursor-pointer'>
                    ...
                    <div x-show="showModal"
                        ...
                        <div x-show="showModal" class="bg-white rounded-xl shadow-2xl p-6 sm:w-10/12 lg:w-5/12 mx-10"
                            ...
                            <div class="mx-auto pl-5 py-5 text-black">
                                ...
                                <a class='text-green-500 send-email border-b hover:text-red-500 sent-email border-dotted @if($errors->has('phone_number') || session()->has('email-message') || !auth()->user()->phone_number)) hidden @endif border-gray-700 cursor-pointer'
                                   href="{{route('login.send_phone_verification')}}">
                                    {{__('Отправить новый код для подтверждения телефонный номер')}}</a><br>


                                <a class='text-green-500 hover:text-red-500 border-b border-dotted border-gray-700 @if($errors->has('phone_number') || session()->has('email-message') || !auth()->user()->phone_number) ) hidden @endif change-email cursor-pointer'>
