...
@if((Illuminate\Support\Facades\Cache::get('date') && Illuminate\Support\Facades\Cache::get('date_1')) != null)
    ...
    <x-laravelYajra tableId="example" stateSave="true" :dtColumns=$dtColumns :dtHeaders=$dtHeaders dom='QBlfrtip' serverSide="true" getData="{{ route('report') }}" tableTitle="{{ __('Отчет') }}" startDate="{{request()->input('date')}}" endDate="{{request()->input('date_1')}}" language="ru"></x-laravelYajra>
