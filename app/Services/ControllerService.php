...
class ControllerService
{
    ...
    public function my_tasks($authUser){
        $item = new MyTaskItem();
        /*$item ->user = auth()->user();*/
        $item->tasks = $authUser->tasks()->orderBy('created_at', 'desc')->get();
        $item->perform_tasks = $authUser->performer_tasks()->orderBy('created_at', 'desc')->get();
