...
//
    public function pay(Request $request){
        $clickuz=new ClickuzService();
        $item=$clickuz->pay($request);
        return redirect()->to("https://my.click.uz/services/pay?service_id=$item->service_id&merchant_id=$item->merchant_id&amount=$item->amount.00&transaction_param=$item->article_id&return_url=$item->return_url");
    }
...
//    }
    ...
    public function prepare(Request $request){
        ...
        $clickuz= new ClickuzService();
        $item=$clickuz->prepare($request);
        return ['click_trans_id' => $item->click_trans_id,'merchant_trans_id' => $item->merchant_trans_id,'merchant_prepare_id' => $item->merchant_prepare_id,'error' => $item->error,'error_note' => $item->error_note];
    }
...
//    }
    ...
    public function complete(Request $request){
       ...
       $clickuz = new ClickuzService();
        $new_complete=$clickuz->complete($request);
...
//    }
    ...
    public function statusup($new_complete){
        ...
        $clickuz=new ClickuzService();
        $item=$clickuz->statusup($new_complete);
        return ['click_trans_id' => $item->click_trans_id,'merchant_trans_id' => $item->merchant_trans_id,'merchant_confirm_id' => $item->merchant_confirm_id,'error' => $item->error,'error_note' => $item->error_note];
